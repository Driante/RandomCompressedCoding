...
function Nvsσ(Vdict::Dict,N::Int64,σVec::Array{Float64},V_l::Array{Float64},PP::Array{Float64},η::Float64;nets=4)
    ...
    println("N=",N," η= ",η, "ε_o=",  minimum(ε),"ε_l=" , minimum(ε_l))
    return ε,ε_l
end
...
#Generate Linear Tuning curves from fitting
V_l,PP,R2 = linear_fit(Vdict[last(σVec)],x_test); V_l = vcat(V_l'...);
V_l ./= sqrt.((var(V_l,dims=2))); V_l  .-=mean(V_l,dims=2)
NVec = 20:10:150; ηVec  = 0.7:0.2:0.8;
ε =[[Nvsσ(Vdict,N,collect(σVec[1:2:end]),V_l,PP,η,nets=1) for N =NVec] for η=ηVec];
