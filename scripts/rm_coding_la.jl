...
# #Fill NaN response with corresponding linear model
# r[isnan.(r)] = r_linear[isnan.(r)];
# r_linear = standardize(ZScoreTransform,r_linear)
# #Standardize firing rate, to fit with our model
# complexity_data = vec(complexity(r,tP,36));complexity_linear = vec(complexity(r_linear[:,:],tP,36))
# hcmpx_d = fit(Histogram,complexity_data); hcmpx_l =  fit(Histogram,complexity_linear)
#Generate network response to the same set of stimuli
