...
##Compute tuning curves for the 3D model RFFN, adapted to data of Lalazaretal.
...
function compute_tuning_curves_net(W::AbstractArray,σVec,x_test;s=0.1,B=0,f2=identity)
        #Compute tuning curves for different values of tuning width
...
x_test_min = -40.; x_test_max=40.; ntest = 21;
x_test = build_grid(x_test_min,x_test_max,ntest);
#Alternative:  use stimuli from data
~,~,tP= import_and_clean_data();
x_test = tP[1];
ntest,~ =size(x_test)
...
##Compute complexity histogram, R2 fit for the simulated tuning curves
